...
if(!class_exists('VsiiAssets'))
{
    class VsiiAssets
    {
        ...
        static function _init()
        {
            ...
            self::$asset_url=VsiiConfig::get('asset_url');
        }

        static function url($file=false)
        {
            return self::$asset_url.'/'.esc_attr($file);
        }
        ...
        {
            add_action('wp_enqueue_scripts',array(__CLASS__,'_action_footer_css'));
        }

        static function _action_footer_css(){

            wp_add_inline_style('Vsii-custom',self::$inline_css);
            ...
            if ( is_array( $value ) ) {
                ...
                if ( isset( $value[0] ) && isset( $value[1] ) ) {
                    ...
                    if ( ! empty( $value['background-image'] ) ) {
                        ...
                        $bg[] = 'url("' . esc_url($value['background-image']) . '")';
