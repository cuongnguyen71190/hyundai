		...
		case "audio":
			if ( $media_url = get_post_meta( get_the_ID(), 'media_url', true ) ) {
				...
				<div class="blog-img-detail">
					...
					<?php esc_html_e('Your browser does not support the audio element.',"vsii-template")?>
				...
				if ( ! empty( $gallery ) ) {
					...
					<div class="single-slider-v2"
						...
						<?php foreach ( $gallery as $key => $value ) {
							...
							echo '<a href="' . esc_url($url[0]) . '" data-lightbox="blog-item3-images">'.balanceTags($img).'</a>';
						} ?>
